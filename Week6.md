...
- 정의: 서로 다른 데이터 테이블을 연결하는 것
- 공통적으로 종재하는 컬럼이 있다면 JOIN할 수 있음
- 보통 id값을 Key로 많이 사용하고, 특정 범위(ex: Date)로도 JOIN 가능
...
### 1 : 트레이너가 보유한 포켓몬들은 얼마나 있는지 알 수 있는 쿼리를 작성하세요
```sql
SELECT 
  kor_name,
...
### 3 : 트레이저의 고향과 포켓몬을 포획한 위치를 비교하여, 자신의 고향에서 포켓몬을 포획한 트레이너의 수를 계산해라
